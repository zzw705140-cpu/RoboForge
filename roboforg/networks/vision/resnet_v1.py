...
from roboforg.data.image_augmentations import resize
from roboforg.networks.vision.film_conditioning_layer import FilmConditioning
ModuleDef = Any
...
class ResNetEncoder(nn.Module):
    ...
    # 常规 ResNet 默认保留梯度并继续执行后续池化/投影；只有 *-frozen 配置显式冻结特征图。
    ...
    def __call__(
        ...
        # EncodingWrapper uses one common interface for regular and pretrained
        # encoders. If features were encoded upstream, pass them through.
        if not encode:
            return observations
